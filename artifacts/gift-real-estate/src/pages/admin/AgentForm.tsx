...
export default function AgentForm() {
  ...
  const field = (label: string, key: keyof Agent, multiline = false) => (
    <div>
      ...
      {multiline ? (
        <textarea rows={4} value={String(form[key] ?? "")} onChange={(e) => set(key, e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:border-[#E31E24] resize-y" />
      ) : (
