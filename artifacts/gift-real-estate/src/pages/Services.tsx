            ...
            {services.map((service) => (
              <div key={service.id} className="bg-white rounded-sm shadow-sm border border-gray-100 hover:shadow-xl hover:border-[#E31E24]/30 transition-all group overflow-hidden flex flex-col">
                {service.image ? (
                  ...
                  <div className="aspect-[16/9] bg-[#1A1A1A]/5 flex items-center justify-center">
                    <Layers size={40} className="text-[#E31E24]/30" />
                  </div>
                )}
                <div className="p-8 flex flex-col flex-grow">
                  <h3 className="font-serif text-2xl font-bold text-[#1A1A1A] mb-4">{service.title}</h3>
                  <p className="text-gray-600 leading-relaxed mb-6 flex-grow">{service.description}</p>
                  <Link href="/contact" className="inline-flex items-center gap-2 text-[#E31E24] font-bold hover:text-[#E31E24] transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              {[
                ...
                <div key={i} className="text-center">
                  <div className="text-5xl font-serif font-black text-[#E31E24] mb-4 opacity-50">{process.step}</div>
                  <h4 className="font-bold text-xl text-[#E31E24] mb-2">{process.title}</h4>
