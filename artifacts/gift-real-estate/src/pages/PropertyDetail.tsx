  ...
  if (isError || !listing) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-20 text-center">
        <h1 className="font-serif text-4xl font-bold text-[#1A1A1A] mb-4">Property Not Found</h1>
        <p className="mb-8 text-gray-600">The property you are looking for does not exist or has been removed.</p>
        <Link href="/properties" className="bg-[#E31E24] text-white px-6 py-2 rounded-sm">Back to Properties</Link>
      </div>
    );
  }
    ...
    <div ref={pageRef} className="min-h-screen bg-[#FFFFFF] pt-24 pb-20">
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="flex gap-2 text-sm text-gray-500 mb-6">
          <Link href="/" className="hover:text-[#E31E24]">Home</Link>
          <span>/</span>
          <Link href="/properties" className="hover:text-[#E31E24]">Properties</Link>
          <span>/</span>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <span className={`px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-sm ${listing.type === "sale" ? "bg-[#E31E24] text-white" : "bg-[#E31E24] text-[#1A1A1A]"}`}>
                For {listing.type}
              </span>
              {listing.status && (
            ...
            <h1 className="font-serif text-3xl md:text-5xl font-bold text-[#1A1A1A] mb-2">{listing.title}</h1>
            <div className="flex items-center gap-2 text-gray-500 text-lg">
              <MapPin size={20} className="text-[#E31E24]" />
              <span>{listing.neighborhood}, {listing.location}, Addis Ababa</span>
            </div>
          </div>
          <div className="text-right">
            <div className="font-serif text-4xl font-bold text-[#E31E24]">{formattedPrice}</div>
          </div>
        </div>
        ...
        <div data-reveal className="mb-12">
          ...
          <div className="grid grid-cols-4 md:grid-cols-6 gap-4">
            {listing.images.map((img, idx) => (
              <button key={idx} onClick={() => setActiveImage(idx)} className={`aspect-video rounded-sm overflow-hidden border-2 transition-all bg-[#1A1A1A] ${activeImage === idx ? "border-[#E31E24] opacity-100" : "border-transparent opacity-60 hover:opacity-100"}`}>
            ...
            <div className="flex flex-wrap gap-6 py-6 border-y border-gray-200 mb-8 bg-white px-8 rounded-sm shadow-sm">
              {listing.bedrooms > 0 && (
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-2 font-serif text-xl font-bold text-[#1A1A1A]">
                    <Bed className="text-[#E31E24]" /> {listing.bedrooms}
                  </div>
                </div>
              )}
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 font-serif text-xl font-bold text-[#1A1A1A]">
                  <Bath className="text-[#E31E24]" /> {listing.bathrooms}
                </div>
              </div>
              <div className="w-px bg-gray-200 hidden md:block" />
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 font-serif text-xl font-bold text-[#1A1A1A]">
                  <Square className="text-[#E31E24]" /> {listing.sizeSqm} sqm
                </div>
              </div>
              <div className="w-px bg-gray-200 hidden md:block" />
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-2 font-serif text-xl font-bold text-[#1A1A1A]">
                  <Calendar className="text-[#E31E24]" /> {new Date(listing.dateAdded).toLocaleDateString("en-US", { month: "short", year: "numeric" })}
                </div>
              </div>
            </div>

            <div className="mb-12">
              <h2 className="font-serif text-2xl font-bold text-[#1A1A1A] mb-4">About this property</h2>
              ...
              {listing.mapsUrl ? (
                <div className="space-y-4">
                  <div className="w-full rounded-sm overflow-hidden border border-gray-200 bg-gray-50 flex flex-col items-center justify-center py-10 gap-4">
                    <MapPin size={40} className="text-[#E31E24]" />
                    <div className="text-center">
                      <p className="font-semibold text-[#1A1A1A] text-lg">{listing.neighborhood}, {listing.location}</p>
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-2 bg-[#E31E24] text-white px-6 py-3 rounded-sm font-bold text-sm hover:bg-[#1A1A1A] transition-colors shadow-md"
                    >
                      <MapPin size={16} />
                ...
                <div className="w-full h-64 bg-gray-100 rounded-sm border border-gray-200 flex flex-col items-center justify-center text-gray-400">
                  <MapPin size={36} className="mb-3 text-[#E31E24]/30" />
        ...
        {relatedListings.length > 0 && (
          <div className="mt-24 pt-12 border-t border-gray-200">
            <h2 className="font-serif text-3xl font-bold text-[#1A1A1A] mb-8">Similar Properties</h2>
